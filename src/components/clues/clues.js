...
export class Clues extends Element {
  ...
  #handleMouseDown = ({ target }) => {
    const clue = this.#getClueByRef(target.closest(`.${ClassName.Clue}`));
    if (!clue) {
      return;
    }
    clue.toggleDiscard();
  ...
  #appendClues = (cluesMx) => {
    ...
    const allClues = cluesMx.map((row, rowIdx) => {
      const cluesList = new Element({ className: ClassName.CluesList });
  ...
  highlight(rowIdx, force = true) {
    this.children[rowIdx]?.toggleClass(ClassName.CluesListHighlighted, force);
  }
